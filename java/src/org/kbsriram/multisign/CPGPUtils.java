...
public class CPGPUtils
{
    ...
    public final static boolean verifyDetachedSignature
        (PGPPublicKey signkey, InputStream srcin, InputStream sigin)
        throws IOException, PGPException, SignatureException
    {
        PGPObjectFactory pgpf = new PGPObjectFactory(sigin);
        PGPSignature sig = CPGPUtils.expectSignature(pgpf);
        if (pgpf.nextObject() != null) {
            throw new IOException("trailing garbage in signature");
        }
        if (sig.getSignatureType() != PGPSignature.BINARY_DOCUMENT) {
            throw new PGPException("Incorrect signature type");
        }
        if (sig.getKeyID() != signkey.getKeyID()) {
            throw new PGPException
                ("signature not signed by correct person");
        }
        sig.init(s_provider, signkey);
        byte[] buf = new byte[8192];
        int nread;
        while ((nread = srcin.read(buf)) > 0) {
            sig.update(buf, 0, nread);
        }
        return sig.verify();
    }

    public final static void writeDetachedSignature
        (PGPPrivateKey signkey, int algorithm,
         InputStream srcin, OutputStream signout)
        throws IOException, PGPException, SignatureException
    {
